/**
 * STATIC para client+server. NO toca Payload — solo data + helper de
 * merge. El wrapper async (server-only) vive en `product-ui-content.ts`.
 * Separar evita que Next.js intente empaquetar `node:fs` cuando el
 * client component ProductCarousel cae al STATIC.
 */
...
export function isFilled(v: unknown): v is string {
  return typeof v === 'string' && v.trim().length > 0
}

export function mergeWithFallback(doc: any, fallback: ProductUiCopy): ProductUiCopy {
